    ...
    if (warpgroup_idx == 0) {
        ...
        if (warp_idx == 4 && elect_one_sync()) {
            ...
            uint8_t* k_scales_ptr =
                OrigKVFormat::IS_V32 ?
                (uint8_t*)params.kv + OrigKVFormat::QUANT_BYTES :
                (uint8_t*)params.kv + params.page_block_size*TMA_K_STRIDE;
            uint8_t* extra_k_scales_ptr =
                ExtraKVFormat::IS_V32 ?
                (uint8_t*)params.extra_kv + ExtraKVFormat::QUANT_BYTES :
                ...
                auto process_one_block = [&](int block_idx, int2 my_indices, auto is_extra_block_t) {
                    ...
                    for (int i = 0; i < 2; ++i) {
                        ...
                        if constexpr (F::IS_V32) {
                            ...
                            *(uint64_t*)scales[i] = *(uint64_t*)(res);
                        } else {
                    ...
                    if constexpr (SCALE_SMEM_STRIDE == F::NUM_SCALES_EACH_TOKEN * F::SCALE_SMEM_BYTES) {
                        ...
                        CUTE_UNROLL
                        for (int i = 0; i < 2; ++i)
                            copy_bytes<F::NUM_SCALES_EACH_TOKEN * F::SCALE_SMEM_BYTES>(plan.scales[rs.index_buf_idx][lane_idx*2 + i], scales[i]);
                    }
