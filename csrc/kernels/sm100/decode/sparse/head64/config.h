...
// One row of scales per token, SCALE_SMEM_BYTES each. In a kernel with an fp4 extra_kv the rows are 32 B and the fp8 tokens use
// their first 16 B
static constexpr int SCALE_SMEM_STRIDE = std::max(
...
struct SharedMemoryPlan {
    ...
    CUTE_ALIGNAS(16) uint8_t scales[NUM_INDEX_BUFS][B_TOPK][SCALE_SMEM_STRIDE];   // bf16 (V3.2), ue8m0 (fp8) or e4m3 (fp4), see KVBlockDequantizer
