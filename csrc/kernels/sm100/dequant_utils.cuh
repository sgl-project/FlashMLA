...
//  - Scales are F::SCALE_SMEM_BYTES each (ue8m0 for fp8, e4m3 for fp4; V3.2's fp32 scales are converted to bf16 by the index
//    warp -- ue8m0 would drop their mantissa and cost accuracy), SCALE_SMEM_STRIDE apart, this CTA's NUM_SCALES first.
...
struct KVBlockDequantizer {
    ...
    static constexpr int SCALE_LOAD_BYTES = (NUM_SCALES * F::SCALE_SMEM_BYTES + 3) / 4 * 4;   // Whole words (the scale rows are padded, see KVCacheFormat)
        ...
        for (int local_row_idx = 0; local_row_idx < ROWS_PER_GROUP; ++local_row_idx) {
            ...
            for (int local_col_idx = 0; local_col_idx < NUM_STEPS; ++local_col_idx) {
                ...
                if constexpr (F::IS_FP4) {
                    ...
                    // V3.2's scales reach shared memory as bf16, see KVCacheFormat::SCALE_SMEM_BYTES
                    ku::nvbf16x2 scale;
                    if constexpr (F::IS_V32) {
                        scale = __bfloat162bfloat162(((const __nv_bfloat16*)scales_row)[scale_idx]);
                    } else {
                        scale = ue8m0_to_bf16x2(((__nv_fp8_e8m0*)scales_row)[scale_idx]);
                    }
                    CUTE_UNROLL
                    for (int i = 0; i < 4; ++i) {
                        data_bf16[i] = __hmul2(fp8x2_to_bf16x2(((ku::nve4m3x2*)&data)[i]), scale);
                    }
                }
